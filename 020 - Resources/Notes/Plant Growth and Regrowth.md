---
up: 
tags:
  - concept
Summary: 
aliases: 
Status:
---
# Plants in Southern Livestock production
~
[[Temperate C3 plants]]

# Plants in Tropical Livestock Production
~
[[Tropical C4 plants]]
# Seasonal Plant Growth of an Annual Growing Plant
~
Autumn: vegetative phase
Winter: vegetative/reproductive phase
Spring: Reproductive/grain development phase
Summer: seed/grain development phase
![](https://i.imgur.com/fXjz2xd.png)

# Plant Phases, Energy and Protein
*What plant state phase should you stay in and why?*
~
Maintain pasture in vegetative state (High energy and digestibility)
![](https://i.imgur.com/OPXmMrv.png)

# Maximising Pasture Growth
~
Irrigation, P and N fertilisers like super phosphate, in [[Gippsland]] - Dairy
# Examples
[[Perennial Pastures]]
[[Pasture Management and Pasture in Extensive Production Systems]]

# Measuring and Forecasting Pasture Growth
~
PGR - Pasture growth rate. Guestimate based on historical figures and current season and projected environmental variables using models regularly
Consider models on soil type, pasture levels and rainfall likelihood to determine stocking rate

*Which of the following seasons has the most consistent pasture growth in SE Australia (ie most commonly the same amount)?*
~
Winter

## Historical PGR
*How to obtain?*
~
Satellite PGR imagery can achieve similar graphs without on farm work
MLA and CiboLabs provide region based data